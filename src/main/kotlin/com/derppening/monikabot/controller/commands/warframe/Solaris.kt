...
import com.derppening.monikabot.impl.warframe.SolarisService
import com.derppening.monikabot.util.helpers.EmbedHelper.sendEmbed
import com.derppening.monikabot.util.helpers.HelpTextBuilder.buildHelpText
import sx.blah.discord.handle.impl.events.guild.channel.message.MessageReceivedEvent

object Solaris : IBase, ILogger {
    override fun cmdName(): String = "warframe-solaris"

    override fun handler(event: MessageReceivedEvent): CommandInterpreter.HandleState {
        val args = getArgumentList(event.message.content).drop(1)

        when {
            args.isEmpty() -> displayBounties(event)
    ...
    private fun displayBounties(event: MessageReceivedEvent) {
        SolarisService.getBountyEmbeds().forEach {
            sendEmbed(it to event.channel)
        }
    }

    override fun help(event: MessageReceivedEvent, isSu: Boolean) {
        buildHelpText(cmdName(), event) {
            description { "Displays Solaris-related information" }
        }
    }
}
